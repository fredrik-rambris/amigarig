...
def flatten_chain(registry: Registry, name: str, _visiting: tuple[str, ...] = ()) -> list[dict]:
    """Resolve `name`'s `extends:` chain (str or list, multiple inheritance
    allowed) into the ordered list of raw per-file dicts that make it up,
    parents first, most specific (own) last -- unmerged, `+`/`^` prefixes
    intact."""
...
def resolve_chain(registry: Registry, name: str) -> dict:
    ...
    return merge_chain(flatten_chain(registry, name))
